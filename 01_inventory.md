...
- **Tenant**：租户，SaaS 中用于组织级隔离与计费的一级边界
- **RBAC**：基于角色的访问控制
- **DLQ**：Dead Letter Queue，失败消息归档队列
- **RPO/RTO**：数据恢复点目标 / 恢复时间目标

### 8.2 假设边界

1. 本次评估基于仓库静态代码与配置，不含外部私有运行平台配置。
2. `.git`、二进制缓存文件不计入业务审计范围（但安全暴露会在风险中提示）。
3. 运行态参数（如 Redis Stream 保留策略、MinIO 服务端加密）若代码不可见，标记为“待验证项”。
